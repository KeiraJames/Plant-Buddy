...
if MONGO_URI and MONGO_URI != "mongodb+srv://recent:recent@cluster0.i7fqn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0": # Check if it's not the default placeholder if you want to avoid attempting connection with it
    try:
        mongo_client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
        mongo_client.admin.command('ping')
        db = mongo_client['temp_moisture'] # Your DB name
        sensor_collection = db['c1']       # Your Collection name
    ...
    # This is the placeholder URI, assume it's not a real connection for now
    # You might want to log this or handle it as "MongoDB not configured"
    print("Using placeholder MONGO_URI. Live sensor data will use defaults if no real connection is established.")
    pass # Allow app to run, sensor data will use defaults
...
def get_ring_html_css():
    ...
    /* Specific Moisture Condition Display (for the smaller section) */
    .moisture-condition-section {{ text-align: center; margin-top: 15px; padding: 10px; background-color: #232323; border-radius: 8px; }}
    ...
    .health-assessment-left {{ text-align: center; }}
    .health-assessment-emoji {{ font-size: 3.5em; line-height: 1; margin-bottom: -5px;}} /* Adjusted for heart emoji */
    .health-assessment-score {{ font-size: 2.8em; font-weight: bold; line-height: 1; margin-top: 0px; }}
    .health-assessment-caption {{ font-size: 0.9em; color: #A3A3A3; margin-top: -2px; }}
    .health-assessment-right .status-text {{ font-size: 1.5em; margin-bottom: 5px; }}
    .health-assessment-right .status-caption {{ font-size: 0.9em; color: #D3D3D3; margin-bottom: 10px;}} /* Lighter grey for caption */
    /* General Health Score Heart (can be reused by Overall Health and Home Cards) */
    .health-score-heart {{ font-size: 1.5em; transition: color 0.5s ease; }} /* Default size for home cards */
    .health-good {{ color: #28a745; }}
    .health-medium {{ color: #ffc107; }} /* Amber/Yellow for Good */
    .health-bad {{ color: #dc3545; }}   /* Red for Needs Attention / Fair */
...
class PlantNetAPI: 
    ...
    def identify_plant_from_bytes(self, image_bytes, filename="image.jpg"):
        if not self.api_key or self.api_key == "2b10X3YLMd8PNAuKOCVPt7MeUe" or self.api_key == "your_plantnet_api_key_here": # Check against specific placeholder
            return {'error': "PlantNet API Key is placeholder or missing."}
        # Actual API call logic would be here
        return {'scientific_name': 'Real Plantus', 'common_name': 'Real Plant', 'confidence': 90.0, 'raw_data': {}}
...
def identify_plant_wrapper(image_bytes, filename="uploaded_image.jpg"): # Unchanged
    if PLANTNET_API_KEY == "2b10X3YLMd8PNAuKOCVPt7MeUe" or PLANTNET_API_KEY == "your_plantnet_api_key_here":
...
def send_message_to_gemini(messages_for_api, image_bytes=None, image_type="image/jpeg"): # Unchanged
    if GEMINI_API_KEY == "AIzaSyCd-6N83gfhMx_-D4WCAc-8iOFSb6hDJ_Q" or GEMINI_API_KEY == "your_gemini_api_key_here": return "Chat disabled: Gemini API Key is placeholder or missing."
...
def get_chat_response(plant_care_info_dict, plant_id_result_dict, chat_history_list, current_user_prompt, image_bytes_for_chat=None, image_type_for_chat="image/jpeg"): # Unchanged
    if GEMINI_API_KEY == "AIzaSyCd-6N83gfhMx_-D4WCAc-8iOFSb6hDJ_Q" or GEMINI_API_KEY == "your_gemini_api_key_here": return "Chat feature disabled."
    ...
    user_prompt_parts = [{"text": current_user_prompt}]; messages_for_api.append({"role": "user", "parts": user_prompt_parts})
...
def find_care_instructions(plant_name_id, care_data_list, threshold=75): # Unchanged (condensed for brevity)
...
def display_identification_result_summary(result): # Unchanged
    ...
    label_style, sci_name, common_name, conf = "font-weight:bold;display:inline-block;width:150px;", result.get('scientific_name','N/A'), result.get('common_name','N/A'), result.get('confidence',0)
...
def display_care_instructions_details(care_info): # Unchanged
    if not care_info: st.warning("No detailed care information available."); return; name = care_info.get('Plant Name', 'This Plant'); st.subheader(f"🌱 {name} Care Guide")
    ...
    left_keys, right_keys = ["☀️ Light","💧 Water","🌡️ Temp","💦 Humidity"], ["🍃 Feeding","흙 Soil","🪴 Potting","⚠️ Toxicity"]; col1,col2=st.columns(2)
    with col1:
        for lbl in left_keys:
            val=care_info.get(details_map[lbl]); 
            if val and str(val).strip() and str(val).lower()!='n/a': st.markdown(f"**{lbl}**"); st.caption(str(val))
    with col2:
        for lbl in right_keys:
            val=care_info.get(details_map[lbl]); 
            if val and str(val).strip() and str(val).lower()!='n/a': st.markdown(f"**{lbl}**"); st.caption(str(val))
    add_care = care_info.get(details_map["✨ Pro Tips"])
    if add_care and str(add_care).strip() and str(add_care).lower()!='n/a':
        with st.expander("✨ Pro Tips",expanded=False): st.markdown(str(add_care))
def find_similar_plant_matches(id_r, care_data_list, limit=3, score_thresh=60): # Unchanged (condensed)
    ...
    final_suggestions, seen_sugg_names, orig_id_name = [], set(), id_r.get('common_name',id_r.get('scientific_name','')).lower()
    for match_data in sorted_matches:
        ...
        if p_sugg_name not in seen_sugg_names: final_suggestions.append(p_info); seen_sugg_names.add(p_sugg_name)
...
def display_suggestion_buttons_for_id_flow(suggestions, care_data): # Unchanged
    ...
    for i, p_info in enumerate(suggestions):
        ...
        if cols[i].button(p_name,key=f"id_sugg_btn_{i}",help=tip,use_container_width=True):
            ...
            st.session_state.current_id_result=new_id_res;st.session_state.current_id_care_info=p_info
...
def get_moisture_condition_from_raw(raw_moisture_value): # Updated condition text
    ...
    if 0 <= raw_moisture_value <= 399: return "Needs Water", "#FF4B3A", "💧" # Red-Orange
    elif 400 <= raw_moisture_value <= 699: return "Okay", "#FFC107", "👍"     # Amber
    elif 700 <= raw_moisture_value <= 999: return "Great!", "#28A745", "✅"  # Green
    elif raw_moisture_value >= 1000: return "Over Watered", "#17A2B8", "⚠️" # Teal
    else: return "Check Sensor", "#6C757D", "🔧" # Grey
...
def calculate_health_score_component(value, ideal_min, ideal_max, lower_is_better=False): # Unchanged
    ...
    if lower_is_better:
        ...
        range_factor = 0.5 # How far outside ideal range to penalize fully (e.g., 50% of ideal_min/max)
        if value < ideal_min:
            range_below = ideal_min - (ideal_min * range_factor); 
            if range_below <= 0: range_below = ideal_min / 2 if ideal_min > 0 else 50 # Avoid zero/negative
            penalty = ((ideal_min - value) / range_below) * 100
            return max(0, 100 - penalty)
        if value > ideal_max:
            range_above = (ideal_max * (1 + range_factor)) - ideal_max; 
...
def calculate_overall_health(moisture_percent, temp_fahrenheit, last_check_ts, care_info): # Unchanged
    scores = []
    scores.append(calculate_health_score_component(moisture_percent, 40, 80)) # Ideal moisture % example
    ...
    elif overall_score >= 60: status_text = "Good" # "Good" maps to amber/yellow now
    elif overall_score >= 40: status_text = "Fair"   # "Fair" might map to red/orange
    return round(overall_score), status_text

def get_health_score_emoji_html(score): # For the main health display emoji
    heart_class = "health-bad"; heart_symbol = "💔" # Default: Needs Attention / Fair
    if score >= 80: heart_class = "health-good"; heart_symbol = "❤️" # Excellent
    elif score >= 60: heart_class = "health-medium"; heart_symbol = "💛" # Good (Yellow)
    # Fair will also use 'health-bad' (red) if not 'Good' or 'Excellent'
    animation_style = "animation: pulse_green 1.5s infinite;" if heart_class == "health-good" else ""
    # Returns only the emoji and score, "Overall Health" caption handled in Markdown
    return f'<span class="health-assessment-emoji {heart_class}" style="{animation_style}">{heart_symbol}</span><div class="health-assessment-score">{score:.0f}%</div>'
...
def initialize_session_state_V2(): # Unchanged
    ...
    if not st.session_state.saved_photos: # Preload examples
...
# =======================================================
def render_home_page(care_data): # Unchanged from previous version with overall health
    st.header("🌿 Plant Buddy Dashboard"); st.markdown("Welcome! Manage your plants, identify new ones, and track their health."); st.divider(); st.subheader("🔍 Quick Actions")
        ...
        if recent_nicks: 
            ...
            for i, nick in enumerate(recent_nicks): 
                ...
                with cols_home[i], st.container(border=True): 
                    ...
                    RAW_MOISTURE_MIN_EXPECTED_HOME, RAW_MOISTURE_MAX_EXPECTED_HOME, MOISTURE_PERCENTAGE_INVERTED_HOME = 200, 700, False # Example defaults
                    ...
                    # Use smaller heart for home page cards via CSS for .health-score-heart
                    st.markdown(f"<p style='text-align: center;'><span class='health-score-heart {('health-good' if overall_score >= 80 else ('health-medium' if overall_score >= 60 else 'health-bad'))}'>{('❤️' if overall_score >= 80 else ('💛' if overall_score >= 60 else '💔'))}</span> {overall_score:.0f}%</p>", unsafe_allow_html=True)
                    if st.button("View Details", key=f"home_view_{nick.replace(' ','_')}", use_container_width=True):
                        st.session_state.viewing_saved_plant_nickname = nick; st.session_state.current_nav_choice = "🪴 My Plants"; st.rerun()
def render_identify_page(care_data): # Unchanged from previous full code (condensed for brevity)
...
def render_my_plants_page(care_data): # Unchanged from previous version with overall health (condensed)
            ...
            for j in range(num_g_cols):
                if i+j < len(sorted_nicks):
                    ...
                    with cols[j],st.container(border=True):
                        ...
                        st.markdown(f"<p style='text-align: center;'><span class='health-score-heart {('health-good' if overall_score_g >= 80 else ('health-medium' if overall_score_g >= 60 else 'health-bad'))}'>{('❤️' if overall_score_g >= 80 else ('💛' if overall_score_g >= 60 else '💔'))}</span> {overall_score_g:.0f}%</p>", unsafe_allow_html=True)
...
def render_plant_health_stats_tab(plant_data_dict, plant_nickname):
    ...
    MOISTURE_PERCENTAGE_INVERTED = False # True if lower raw value means wetter

    st.header("Current Health Assessment") # Changed from subheader to header

    if st.button("🔄 Refresh Live Sensor Data", key=f"refresh_sensor_{plant_nickname}"):
        st.toast("Attempting to fetch latest sensor data...", icon="⏳"); st.rerun() # Added rerun for refresh
    ...
    col_left, col_right = st.columns([1, 2]) # Adjust ratio as needed, e.g. [1,1.5] or [1,2]

    with col_left:
        # get_health_score_emoji_html now returns the emoji and score div
        st.markdown(f"<div class='health-assessment-left'>{get_health_score_emoji_html(overall_health_score)}<div class='health-assessment-caption'>Overall Health</div></div>", unsafe_allow_html=True)
    ...
    with col_right:
        ...
        caption_text = "🌿 Thriving!"
        if overall_health_status_text == "Good": caption_text = "👍 Doing well, keep it up."
        elif overall_health_status_text == "Fair": caption_text = "⚠️ Needs some attention."
        elif overall_health_status_text == "Needs Attention": caption_text = "💔 Urgent care needed!"
        st.markdown(f"<p class='status-caption'>{caption_text}</p>", unsafe_allow_html=True)
        
        st.progress(int(overall_health_score))
        st.markdown("</div>", unsafe_allow_html=True) # Close health-assessment-right div
    ...
    <div class="moisture-condition-section">
        ...
        <div class="moisture-condition-status-caption">Raw sensor value: {raw_moisture_value_current if raw_moisture_value_current is not None else 'N/A'}</div>
    </div>
    ...
    if plant_data_dict['health_history']:
        ...
        if last_val_hist == moisture_percentage_current and (datetime.now(EASTERN_TZ)-last_dt_hist).total_seconds() < 3600: add_hist = False # Only add if value changed or >1hr
    if add_hist:
    ...
    elif isinstance(last_check_timestamp_current, (int, float)): last_check_ts_for_ring = datetime.fromtimestamp(last_check_timestamp_current, tz=timezone.utc)
    else: last_check_ts_for_ring = last_check_timestamp_current
    last_check_ts_for_ring = last_check_ts_for_ring.astimezone(EASTERN_TZ) if last_check_ts_for_ring.tzinfo else EASTERN_TZ.localize(last_check_ts_for_ring)
    ...
    if temp_fahrenheit_current is not None:
        ...
        care_s, temp_rng_str = plant_data_dict.get("care_info",{}), care_s.get("Temperature Range","65-85°F")
        temp_desc = f"Ambient: {temp_fahrenheit_current:.1f}°F. Ideal: {temp_rng_str or 'N/A'}."
    ...
    mins_ago = int((datetime.now(EASTERN_TZ)-last_check_ts_for_ring).total_seconds()/60); fresh_prog = max(0,(1-(min(mins_ago,FRESHNESS_MAX_MINUTES_AGO)/FRESHNESS_MAX_MINUTES_AGO)))*100
    ring3_fresh = generate_ring_html("Last Update", str(mins_ago), "MINS AGO", fresh_prog, FRESHNESS_COLOR, FRESHNESS_TRACK_COLOR, sim_time_rings_display, f"Sensor data updated {mins_ago} mins ago.", 2)
...
# --- Main App Logic ---
def main(): # Condensed for brevity, no major logic changes here
    ...
    if not st.session_state.welcome_response_generated:
        if GEMINI_API_KEY != "AIzaSyCd-6N83gfhMx_-D4WCAc-8iOFSb6hDJ_Q" and GEMINI_API_KEY != "your_gemini_api_key_here":
            payload=[{"role":"user","parts":[{"text":"System: You are Plant Buddy app assistant. Generate a very short, cheerful welcome (1-2 sentences) for the sidebar. Mention plant companion app."}]},{"role":"model","parts":[{"text":"Okay!"}]}]; st.session_state.welcome_response=send_message_to_gemini(payload)
...
if __name__ == "__main__":
    if PLANTNET_API_KEY == "2b10X3YLMd8PNAuKOCVPt7MeUe" or PLANTNET_API_KEY == "your_plantnet_api_key_here": st.toast("PlantNet API Key is placeholder. ID in demo mode.",icon="⚠️")
    if GEMINI_API_KEY == "AIzaSyCd-6N83gfhMx_-D4WCAc-8iOFSb6hDJ_Q" or GEMINI_API_KEY == "your_gemini_api_key_here": st.toast("Gemini API Key is placeholder. Chat limited.",icon="⚠️")
    main()
